...
void remove_from_buffer(int *a, int *b, int *c, two_d_buffer *p)
{
	...
	//fill in code below
	pthread_mutex_lock(&p->mutex);
    while (p->remain == 0) pthread_cond_wait(&p->consume_cond, &p->mutex);
...
{   
	...
	while(!done)
	{
		...
		int item = rand() % 100;
        prepare(item);
        add_to_buffer(item, 0, q);

        pthread_mutex_lock(&p->mutex);
        add_last(&p->head, &p->tail, create_node(item));
        pthread_mutex_unlock(&p->mutex);

		if (my_data->total >= 5 * my_data->id + 5) done = 1;
...
int main(int argc, char *argv[])
{
	...
	//TODO
	//fill in code below
	for (t = 0; t < n_producer; t++) total += thread_data_array[n_consumer + t].total;
