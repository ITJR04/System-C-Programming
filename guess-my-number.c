...
void    child_main(int fdp[], int fdc[], int seed)
{
    ...
    int result; 
    int mx = gmn_get_max();
    int wrte = write(fdc[1],&mx , sizeof(int));
    if (wrte != sizeof(int)){
        die("Could not write.");
    }
    ...
    do {
        if(read(fdp[0], &guess, sizeof(int)) != sizeof(int)){
            die("Could not read");
        }
        result = gmn_check(&gmn, guess);
        if(write(fdc[1], &result, sizeof(int)) != sizeof(int)){
            die("Could not write.");
        }
    } while(result != 0);
    if(write(fdc[1],gmn_get_message(&gmn),strlen(gmn_get_message(&gmn))) != strlen(gmn_get_message(&gmn))){
        die("Could not write.");
    }
...
int main(int argc, char *argv[])
{
 ...
 /*   if (lsof) {
    ...
    if ( read(fdc[0], &max, sizeof(int)) != sizeof(int)){
        die("read failed");
    }
    ...
    do { 
        ...
        if (write(fdp[1], &guess, sizeof(int)) != sizeof(int)){
            die("Could not write.");
        }
        if (read(fdc[0], &result, sizeof(int)) != sizeof(int)){
            die("Could not read.");
        }
        if (result > 0)
            min = guess + 1;
