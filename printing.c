...
{
    // TODO
    printer_t *my_print = arg;
    job_queue_t *jq = my_print->jq;
    ...
    while(!comp){
        ...
        int job = q_fetch_job(jq, my_print->id);
...
int main(int argc, char *argv[])
{
    ...
    for(i = 0; i < num_printers; i++)
    {
        if(pthread_join(printers[i].thread_id, NULL) != 0)
        {
            perror("Error\n");
        }
    }

    if(pthread_mutex_destroy(&job_queue.mutex) != 0)
    {
        perror("Error\n");
    }
